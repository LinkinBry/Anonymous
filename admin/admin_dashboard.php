...
// Now lives in /admin/ subfolder.
// All include paths use __DIR__ . '/../' to reach root files.
// AJAX endpoints (approve_review, get_faculty_reviews, etc.) also moved to /admin/.
...
// ── Helper: resolve photo path relative to this file's location ───────────────
function resolvePhoto(string $path, string $fallbackUrl): string {
    if (empty($path)) return $fallbackUrl;
    // path stored in DB is relative to htdocs root (e.g. uploads/faculty_1.jpg)
...
<div class="sidebar">
    ...
    <nav>
        ...
        <a href="#overview"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg><span class="nav-link-text">Overview</span></a>
        <a href="#users"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg><span class="nav-link-text">Users</span></a>
        <a href="#faculties"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg><span class="nav-link-text">Faculty</span></a>
        <a href="#pending" class="nav-pending-link">
            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            <span class="nav-link-text">Pending Reviews</span>
            <?php if ($pending_count > 0): ?>
            <span class="sidebar-nav-badge" style="background:#E24B4A;"><?php echo $pending_count; ?></span>
            <span class="sidebar-pending-badge"><?php echo $pending_count > 9 ? '9+' : $pending_count; ?></span>
            <?php endif; ?>
        </a>
        <a href="#approved"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg><span class="nav-link-text">Approved Reviews</span></a>
        <a href="#reports"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg><span class="nav-link-text">Reports</span></a>
    </nav>
    <div class="sidebar-footer">
        <!-- Points to admin/logout.php — redirects back to admin/login.php -->
        <a href="logout.php">
...
<div class="main">
    ...
    <div id="overview" class="stats-grid stats-row-4">
        <div class="stat-card s-users"><div class="stat-label">Total Users</div><div class="stat-value"><?php echo $total_users; ?></div><div class="stat-sub">+<?php echo $week_users; ?> this week</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#4338CA" stroke-width="1.8"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg></div></div>
        <div class="stat-card s-faculty"><div class="stat-label">Total Faculty</div><div class="stat-value"><?php echo $total_faculties; ?></div><div class="stat-sub">Across departments</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#B45309" stroke-width="1.8"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg></div></div>
        <div class="stat-card s-pending"><div class="stat-label">Pending Reviews</div><div class="stat-value" id="pendingStatVal"><?php echo $pending_count; ?></div><div class="stat-sub">Needs your action</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#C2410C" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div></div>
        <div class="stat-card s-approved"><div class="stat-label">Approved Reviews</div><div class="stat-value"><?php echo $approved_count; ?></div><div class="stat-sub">Total published</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#065F46" stroke-width="1.8"><polyline points="20 6 9 17 4 12"/></svg></div></div>
    </div>

    <div class="stats-grid stats-row-3">
        <div class="stat-card s-rejected"><div class="stat-label">Rejected</div><div class="stat-value"><?php echo $rejected_count; ?></div><div class="stat-sub">Not published</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#991B1B" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></div></div>
        <div class="stat-card s-total"><div class="stat-label">Total Reviews</div><div class="stat-value"><?php echo $total_reviews; ?></div><div class="stat-sub">All time</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="#6B7280" stroke-width="1.8"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg></div></div>
        <div class="stat-card s-admins"><div class="stat-label">Admins</div><div class="stat-value"><?php echo $total_admins; ?></div><div class="stat-sub">System admins</div><div class="stat-icon"><svg viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" stroke-width="1.8"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div></div>
    </div>
    ...
    <div class="section" id="pending">
        ...
        <div class="bulk-bar" id="review_bulk_bar">
            ...
            <button type="submit" name="bulk_approve" class="btn btn-green" style="padding:8px 10px;font-size:11px;" onclick="return confirm('Approve selected reviews?')"><svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="margin-right:4px;display:inline;"><polyline points="20 6 9 17 4 12"/></svg>Approve Selected</button>
            <button type="submit" name="bulk_reject" class="btn btn-red" style="padding:8px 10px;font-size:11px;" onclick="return confirm('Reject selected reviews?')"><svg width="12" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="margin-right:4px;display:inline;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Reject Selected</button>
        </div>
        ...
        <div class="pend-row admin-pageable-row" data-section="pending" data-text="<?php echo htmlspecialchars(strtolower($r['review_text'].' '.$r['user_fullname'].' '.$r['faculty_name'])); ?>" data-sentiment="<?php echo $s; ?>">
            ...
            <div class="pend-avatar" style="overflow:hidden;padding:0;">
                ...
                <?php else: ?><span style="font-size:13px;font-weight:700;"><?php echo strtoupper(substr($r['user_fullname'], 0, 2)); ?></span>
                <?php endif; ?>
            </div>
            <div class="pend-body">
                <div class="pend-meta">
    ...
    <div class="users-faculty-row">
        <div class="section" id="faculties">
            <div class="section-header">
                <div class="section-title"><svg viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" stroke-width="2" style="width:16px;height:16px"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg>Faculty <span class="section-badge"><?php echo $total_faculties; ?></span></div>
                <div style="display:flex;gap:6px;align-items:center;">
                    ...
                    <button type="button" class="btn btn-maroon" style="font-size:11px;padding:4px 10px;" onclick="document.getElementById('addFacultyModal').classList.add('open')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Add</button>
                </div>
            </div>
            <div class="section-toolbar">
                <div class="search-box" style="flex:1;min-width:0;"><svg width="12" height="12" fill="none" stroke="var(--gray-400)" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" id="faculties-search" placeholder="Search faculty…" oninput="filterFacRows()"></div>
            ...
            <div class="fac-row admin-pageable-row" data-section="faculty" data-text="<?php echo htmlspecialchars(strtolower($f['name'].' '.($f['department']??''))); ?>" data-dept="<?php echo htmlspecialchars(strtolower($f['department']??'')); ?>">
                ...
                <div class="fac-row-av"><?php if ($fphoto): ?><img src="<?php echo htmlspecialchars($fphoto); ?>" alt=""><?php else: ?><?php echo strtoupper(substr($f['name'], 0, 2)); ?><?php endif; ?></div>
                <div style="flex:1;min-width:0;"><div class="fac-row-name"><?php echo htmlspecialchars($f['name']); ?></div><div class="fac-row-dept"><?php echo htmlspecialchars($f['department'] ?? '—'); ?></div></div>
                <div style="text-align:right;flex-shrink:0;">
                    <?php if ($avg > 0): ?>
                    <div class="fac-row-stars"><?php for ($s = 1; $s <= 5; $s++): ?><span class="<?php echo $s <= $full ? 'lit' : ''; ?>">★</span><?php endfor; ?><span class="fac-row-score"><?php echo number_format($avg, 1); ?></span></div>
                    <div style="font-size:10px;color:var(--gray-400);"><?php echo $f['review_count']; ?> reviews</div>
                    <?php else: ?><div style="font-size:10px;color:var(--gray-400);">No ratings yet</div><?php endif; ?>
                </div>
                <div style="margin-left:8px;display:flex;gap:3px;flex-shrink:0;">
                    <button type="button" class="btn btn-outline" style="padding:3px 8px;font-size:10px;" onclick="openFacultyModal(<?php echo $f['id']; ?>,'<?php echo htmlspecialchars(addslashes($f['name'])); ?>','<?php echo htmlspecialchars(addslashes($f['department']??'')); ?>','<?php echo $fphoto ? htmlspecialchars($fphoto) : ''; ?>')">View</button>
        ...
        <div class="section" id="users">
            <div class="section-header">
                <div class="section-title"><svg viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" stroke-width="2" style="width:16px;height:16px"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>Users <span class="section-badge"><?php echo $total_users; ?></span></div>
                <button type="button" id="edit_users_btn" class="btn btn-outline" style="font-size:11px;padding:4px 10px;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit</button>
            </div>
            <div class="section-toolbar"><div class="search-box" style="flex:1;min-width:0;"><svg width="12" height="12" fill="none" stroke="var(--gray-400)" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" id="users-search" placeholder="Search users…" oninput="filterUserRows()"></div></div>
            <form method="POST" id="usersForm">
            <div id="users-rows-wrap">
            <?php while ($u = mysqli_fetch_assoc($users)):
                $upic = resolvePhoto($u['profile_pic'] ?? '', '');
            ?>
            <div class="fac-row admin-pageable-row" data-section="users" data-text="<?php echo htmlspecialchars(strtolower($u['fullname'].' '.$u['username'])); ?>">
                ...
                <div class="fac-row-av">
                    ...
                    <?php else: ?><div style="width:100%;height:100%;border-radius:50%;background:var(--maroon-pale);color:var(--maroon);font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;"><?php echo strtoupper(substr($u['fullname'], 0, 2)); ?></div><?php endif; ?>
                </div>
                <div style="flex:1;min-width:0;"><div class="fac-row-name"><?php echo htmlspecialchars($u['fullname']); ?></div><div class="fac-row-dept">@<?php echo htmlspecialchars($u['username']); ?></div></div>
                <div style="margin-left:8px;display:flex;gap:3px;flex-shrink:0;">
                    ...
                    <button type="button" class="btn btn-red" style="padding:3px 8px;font-size:10px;"
                            onclick="confirmDeleteUser(<?php echo $u['id']; ?>,'<?php echo htmlspecialchars(addslashes($u['fullname'])); ?>')"><svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button>
                </div>
            </div>
            <?php endwhile; ?>
            </div>
            <div id="users-pagination" class="section-pagination"></div>
            <div class="bulk-bar" id="bulk_actions"><span id="selected_count">0 users selected</span><button type="submit" name="bulk_delete_users" class="btn btn-red" style="padding:8px 10px;font-size:11px;" onclick="return confirm('Delete selected users permanently?')">Delete Selected</button></div>
            </form>
        </div>
    </div>
    ...
    <div class="section" id="approved">
        <div class="section-header">
            <div class="section-title"><svg viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" stroke-width="2" style="width:16px;height:16px"><polyline points="20 6 9 17 4 12"/></svg>Approved Reviews <span class="section-badge"><?php echo $approved_count; ?></span></div>
        </div>
        <div class="section-toolbar">
            <div class="search-box" style="min-width:200px;"><svg width="12" height="12" fill="none" stroke="var(--gray-400)" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" id="approved-search" placeholder="Search by faculty or reviewer…" oninput="filterApprovedRows()"></div>
            <select id="approved-faculty-filter" class="filter-sel" style="font-size:11px;" onchange="filterApprovedRows()">
                ...
                while ($frow = mysqli_fetch_assoc($fac_res)): ?><option value="<?php echo htmlspecialchars(strtolower($frow['name'])); ?>"><?php echo htmlspecialchars($frow['name']); ?></option><?php endwhile; ?>
            </select>
        </div>
        <form method="POST">
        <div style="padding:8px 16px;border-bottom:1px solid var(--gray-100);display:flex;align-items:center;gap:8px;background:var(--gray-50);"><input type="checkbox" id="select_all_approved" onchange="document.querySelectorAll('.approved_cb').forEach(c=>c.checked=this.checked);updateApprovedBulk();"><label for="select_all_approved" style="font-size:12px;color:var(--gray-500);cursor:pointer;">Select all visible</label></div>
        <div id="approved-rows-wrap">
        <?php $has_approved = false; while ($r = mysqli_fetch_assoc($approved_reviews)): $has_approved = true; ?>
        <div class="fac-row admin-pageable-row" data-section="approved" data-text="<?php echo htmlspecialchars(strtolower($r['faculty_name'].' '.$r['user_fullname'])); ?>">
            <div style="width:22px;flex-shrink:0;text-align:center;"><input type="checkbox" name="selected_approved[]" value="<?php echo $r['id']; ?>" class="approved_cb" onchange="updateApprovedBulk()"></div>
            <div class="fac-row-av"><div style="width:100%;height:100%;border-radius:50%;background:var(--maroon-pale);color:var(--maroon);font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;"><?php echo strtoupper(substr($r['faculty_name'], 0, 2)); ?></div></div>
            <div style="flex:1;min-width:0;"><div class="fac-row-name"><?php echo htmlspecialchars($r['faculty_name']); ?></div><div class="fac-row-dept"><?php echo htmlspecialchars($r['user_fullname']); ?> · <?php echo date("M j, Y", strtotime($r['created_at'])); ?></div></div>
            <div style="margin-left:8px;display:flex;gap:3px;flex-shrink:0;">
                <button type="button" class="btn btn-outline" style="padding:3px 8px;font-size:10px;" onclick="openModal('<?php echo htmlspecialchars(addslashes($r['review_text'])); ?>','<?php echo htmlspecialchars(addslashes($r['faculty_name'])); ?>','<?php echo htmlspecialchars(addslashes($r['user_fullname'])); ?>',<?php echo intval($r['rating_teaching']); ?>,<?php echo intval($r['rating_communication']); ?>,<?php echo intval($r['rating_punctuality']); ?>,<?php echo intval($r['rating_fairness']); ?>,<?php echo intval($r['rating_overall']); ?>,'<?php echo htmlspecialchars(addslashes($r['review_photo'])); ?>')">View</button>
                <a href="reject_review.php?id=<?php echo $r['id']; ?>" class="btn btn-red" style="padding:3px 8px;font-size:10px;" onclick="return confirm('Delete this approved review?')"><svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></a>
            </div>
        </div>
        <?php endwhile; ?>
        <?php if (!$has_approved): ?><div style="text-align:center;padding:28px;color:var(--gray-400);font-size:13px;">No approved reviews yet.</div><?php endif; ?>
        </div>
        ...
        <div class="bulk-bar" id="approved_bulk_bar"><span id="approved_selected_count">0 reviews selected</span><button type="submit" name="bulk_delete_approved" class="btn btn-red" style="padding:8px 10px;font-size:11px;" onclick="return confirm('Delete selected reviews permanently?')">Delete Selected</button></div>
        </form>
    </div>

    <!-- ══ Reports ════════════════════════════════════════════════════════ -->
    <div class="section" id="reports">
        <div class="section-header"><div class="section-title"><svg viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" stroke-width="2" style="width:16px;height:16px"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>System Reports</div></div>
        <div style="padding:16px 16px 0;">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:9px;margin-bottom:20px;">
                <?php foreach ([['Total Users',$total_users,'#4338CA'],['Admins',$total_admins,'#7C0A02'],['Faculties',$total_faculties,'#B45309'],['Total Reviews',$total_reviews,'#6B7280'],['Pending',$pending_count,'#C2410C'],['Approved',$approved_count,'#065F46'],['Rejected',$rejected_count,'#991B1B']] as $c): ?>
                <div style="background:var(--gray-100);border-radius:var(--radius-sm);padding:11px;border:1px solid var(--gray-200);"><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.4px;color:var(--gray-400);font-weight:600;margin-bottom:4px;"><?php echo $c[0]; ?></div><div style="font-size:20px;font-weight:700;color:<?php echo $c[2]; ?>;"><?php echo $c[1]; ?></div></div>
                <?php endforeach; ?>
            </div>
        </div>
        <div style="padding:0 16px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px;">
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.8px;color:var(--gray-400);font-weight:600;">Weekly Activity</div>
        ...
        <div style="padding:0 16px 16px;border-top:1px solid var(--gray-100);padding-top:14px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px;">
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.8px;color:var(--gray-400);font-weight:600;">Monthly Faculty Summary <span style="font-weight:400;text-transform:none;letter-spacing:0;margin-left:5px;font-size:10px;">Powered by Groq AI</span></div>
                <div style="display:flex;gap:6px;align-items:center;">
            ...
            <div style="background:var(--gray-100);border-radius:var(--radius-sm);padding:14px;border:1px solid var(--gray-200);"><div id="aiSummaryText" style="font-size:13px;color:var(--gray-700);line-height:1.8;"><span style="color:var(--gray-400);">Select a month and click Generate.</span></div></div>
        </div>
    </div>
...
<!-- All modals below are identical to the original admin_dashboard.php modals -->
<!-- ══ User Modal ═══════════════════════════════════════════════════════ -->
<div class="modal-overlay" id="userModal">
    <div class="modal-box" style="max-width:440px;">
        ...
        <div style="padding:28px;text-align:center;">
            <img id="userModalAvatar" src="" alt="" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:3px solid var(--maroon-pale);box-shadow:0 4px 20px rgba(139,0,0,0.15);display:block;margin:0 auto 16px;">
            ...
            <div style="background:var(--gray-100);border-radius:var(--radius-sm);padding:10px;"><div style="display:flex;align-items:center;justify-content:center;gap:7px;font-size:13px;color:var(--gray-600);"><svg width="14" height="14" fill="none" stroke="var(--maroon)" stroke-width="2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg><span id="userModalEmail"></span></div></div>
        </div>
        <div style="padding:0 20px 20px;"><div style="font-size:11px;font-weight:600;color:var(--gray-600);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:10px;padding-bottom:10px;border-top:1px solid var(--gray-100);padding-top:14px;">Reviews Made</div><div id="userReviewsList" style="font-size:12px;color:var(--gray-400);">Loading...</div></div>
    </div>
</div>
...
<div class="modal-overlay" id="deleteUserModal">
    <div class="modal-box" style="max-width:380px;">
        ...
        <form method="POST">
            <div style="padding:28px;text-align:center;"><div style="width:52px;height:52px;border-radius:50%;background:#fee2e2;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;"><svg width="22" height="22" fill="none" stroke="#ef4444" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></div><p style="font-size:14px;font-weight:600;color:var(--gray-800);margin-bottom:6px;">Delete this user?</p><p style="font-size:12px;color:var(--gray-400);">All data for <strong id="deleteUserName"></strong> will be permanently removed.</p><input type="hidden" name="delete_user_id" id="deleteUserIdInput"></div>
            <div style="display:flex;justify-content:center;gap:10px;padding:14px 20px;border-top:1px solid var(--gray-100);"><button type="button" class="btn-secondary" onclick="document.getElementById('deleteUserModal').classList.remove('open')">Cancel</button><button type="submit" name="delete_single_user" class="btn btn-red">Delete User</button></div>
        </form>
    </div>
</div>
    ...
    <div class="modal-box" style="max-width:660px;">
        <div class="modal-head" style="flex-direction:column;align-items:flex-start;gap:0;padding-bottom:0;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;width:100%;padding-bottom:12px;">
                <div style="display:flex;align-items:center;gap:14px;"><div style="position:relative;flex-shrink:0;"><img id="facModalPhoto" src="" alt="" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:3px solid var(--maroon-pale);box-shadow:0 4px 20px rgba(139,0,0,0.15);display:block;"></div><div><h3 id="facModalName" style="color:var(--maroon);font-size:16px;margin-bottom:3px;"></h3><div id="facModalDept" style="font-size:12px;color:var(--gray-400);"></div></div></div>
...
<div class="modal-overlay" id="reviewModal">
    <div class="modal-box">
        <div class="modal-head"><h3><span id="modalFaculty" style="color:var(--maroon);"></span><span style="color:var(--gray-400);font-weight:400;font-size:12px;margin-left:7px;">by <span id="modalUser"></span></span></h3><button class="modal-close" onclick="closeModal()">&times;</button></div>
        <div style="padding:14px 20px 0;" id="modalRatings"></div>
        <div id="modalPhotoWrap" style="padding:10px 20px 0;display:none;"><img id="modalPhotoImg" src="" alt="" style="max-width:100%;max-height:180px;border-radius:9px;object-fit:cover;border:1px solid var(--gray-200);"></div>
...
<!-- Add/Edit Faculty Modals — identical to original, paths unchanged since they POST to same page -->
<div class="modal-overlay" id="addFacultyModal">
    <div class="modal-box" style="max-width:480px;">
        <div class="modal-head"><div style="display:flex;align-items:center;gap:10px;"><div style="width:32px;height:32px;border-radius:50%;background:var(--maroon-pale);display:flex;align-items:center;justify-content:center;"><svg width="15" height="15" fill="none" stroke="var(--maroon)" stroke-width="2" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div><div><h3>Add New Faculty</h3><div style="font-size:11px;color:var(--gray-400);">Fill in the details below</div></div></div><button class="modal-close" onclick="closeAddFacultyModal()">&times;</button></div>
        <form method="POST" enctype="multipart/form-data" onsubmit="return validateAddFaculty()">
            <div style="padding:18px 20px;">
                ...
                <div class="af-group"><label>Profile Photo <span style="color:var(--gray-400);font-weight:400;">(optional)</span></label><div style="display:flex;align-items:center;gap:12px;"><img id="afPhotoPreview" src="https://ui-avatars.com/api/?name=Faculty&background=7C0A02&color=fff&size=56" class="fac-photo-preview" alt="" style="width:52px;height:52px;"><div><input type="file" name="faculty_photo" id="afPhoto" accept="image/jpeg,image/png,image/webp" style="display:none;" onchange="previewFacultyPhoto(this,'afPhotoPreview')"><button type="button" class="btn btn-outline" style="font-size:11px;" onclick="document.getElementById('afPhoto').click()">Upload Photo</button><div style="font-size:10px;color:var(--gray-400);margin-top:3px;">JPG, PNG, WEBP · Max 3MB</div></div></div></div>
                <div class="af-group"><label>Full Name <span style="color:#ef4444">*</span></label><div class="af-field"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><input type="text" name="faculty_name" id="afName" class="af-input" placeholder="e.g. Dr. Juan dela Cruz" required></div></div>
                <div class="af-group">
                    <label>Department <span style="color:#ef4444">*</span></label>
                    <input type="hidden" name="faculty_dept" id="afDept">
                    <div style="display:flex;flex-direction:column;gap:5px;">
                        <?php if (!empty($af_arr)): ?>
                        <div class="af-field" style="position:relative;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);pointer-events:none;color:var(--gray-400);z-index:1;width:14px;height:14px;"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/></svg><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);pointer-events:none;color:var(--gray-400);width:14px;height:14px;"><polyline points="6 9 12 15 18 9"/></svg><select id="afDeptSelect" onchange="afDeptChange(this)" style="width:100%;padding:9px 30px 9px 34px;border:1.5px solid var(--gray-200);border-radius:var(--radius-sm);font-family:'DM Sans',sans-serif;font-size:13px;color:var(--gray-800);outline:none;background:white;appearance:none;-webkit-appearance:none;cursor:pointer;"><option value="">— Select a department —</option><?php foreach ($af_arr as $afd): ?><option value="<?php echo htmlspecialchars($afd); ?>"><?php echo htmlspecialchars($afd); ?></option><?php endforeach; ?><option value="__new__">+ Add new department…</option></select></div>
                        <?php endif; ?>
                        <div id="afNewDeptWrap" style="<?php echo empty($af_arr) ? '' : 'display:none;'; ?>"><div class="af-field"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/></svg><input type="text" id="afNewDeptInput" class="af-input" placeholder="Type new department name…" oninput="document.getElementById('afDept').value=this.value.trim()"></div><?php if (!empty($af_arr)): ?><button type="button" onclick="afCancelNewDept()" style="margin-top:4px;font-size:10px;color:var(--gray-400);background:none;border:none;cursor:pointer;font-family:'DM Sans',sans-serif;padding:0;">← Back to existing departments</button><?php endif; ?></div>
                    </div>
                </div>
            </div>
            <div style="display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid var(--gray-100);background:var(--gray-50);"><button type="button" class="btn-secondary" onclick="closeAddFacultyModal()">Cancel</button><button type="submit" name="add_faculty_modal" class="btn btn-maroon">Add Faculty</button></div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="editFacultyModal">
    <div class="modal-box" style="max-width:480px;">
        <div class="modal-head"><div style="display:flex;align-items:center;gap:10px;"><div style="width:32px;height:32px;border-radius:50%;background:#dbeafe;display:flex;align-items:center;justify-content:center;"><svg width="15" height="15" fill="none" stroke="#1e40af" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></div><div><h3>Edit Faculty</h3><div style="font-size:11px;color:var(--gray-400);">Update faculty details</div></div></div><button class="modal-close" onclick="closeEditFacultyModal()">&times;</button></div>
        <form method="POST" enctype="multipart/form-data" onsubmit="return validateEditFaculty()">
            ...
            <div style="padding:18px 20px;">
                ...
                <div class="af-group"><label>Profile Photo <span style="color:var(--gray-400);font-weight:400;">(optional)</span></label><div style="display:flex;align-items:center;gap:12px;"><img id="efPhotoPreview" src="" class="fac-photo-preview" alt="" style="width:52px;height:52px;"><div><input type="file" name="ef_faculty_photo" id="efPhoto" accept="image/jpeg,image/png,image/webp" style="display:none;" onchange="previewFacultyPhoto(this,'efPhotoPreview')"><button type="button" class="btn btn-outline" style="font-size:11px;" onclick="document.getElementById('efPhoto').click()">Change Photo</button></div></div></div>
                <div class="af-group"><label>Full Name <span style="color:#ef4444">*</span></label><div class="af-field"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><input type="text" name="ef_name" id="efName" class="af-input" placeholder="e.g. Dr. Juan dela Cruz" required></div></div>
                <div class="af-group"><label>Department <span style="color:#ef4444">*</span></label><div class="af-field"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/></svg><input type="text" name="ef_dept" id="efDept" class="af-input" placeholder="e.g. College of Engineering" list="efDeptList" required autocomplete="off"></div><datalist id="efDeptList"><?php foreach ($ef_arr as $efd): ?><option value="<?php echo htmlspecialchars($efd); ?>"><?php endforeach; ?></datalist><?php if (!empty($ef_arr)): ?><div style="font-size:10px;color:var(--gray-400);margin-top:5px;margin-bottom:5px;">Quick select:</div><div class="af-chips"><?php foreach ($ef_arr as $efd): ?><button type="button" class="af-chip" onclick="document.getElementById('efDept').value='<?php echo htmlspecialchars(addslashes($efd)); ?>'"><?php echo htmlspecialchars($efd); ?></button><?php endforeach; ?><button type="button" class="af-chip af-chip-new" onclick="document.getElementById('efDept').value='';document.getElementById('efDept').focus();">+ New dept</button></div><?php endif; ?></div>
            </div>
            <div style="display:flex;justify-content:flex-end;gap:8px;padding:12px 20px;border-top:1px solid var(--gray-100);background:var(--gray-50);"><button type="button" class="btn-secondary" onclick="closeEditFacultyModal()">Cancel</button><button type="submit" name="edit_faculty_modal" class="btn btn-maroon">Save Changes</button></div>
        </form>
    </div>
</div>
...
<!-- AJAX endpoints now in /admin/ subfolder -->
<script>
// Override AJAX fetch URLs to use relative paths (same /admin/ folder)
window.FACULTY_REVIEWS_URL  = 'get_faculty_reviews.php';
window.USER_REVIEWS_URL     = 'get_user_reviews.php';
window.FACULTY_SUMMARY_URL  = 'faculty_summary.php';
</script>

<script src="../assets/js/admin.js"></script>
<script>
// Patch admin.js fetch calls to use the URLs defined above
const origLoadFacultyReviews = window.loadFacultyReviews;
window.loadFacultyReviews = function(fid) {
    ...
    fetch(window.FACULTY_REVIEWS_URL + '?faculty_id=' + fid, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.json()).then(data => { if (data.error === 'session_expired') { window.location.href = 'login.php?timeout=1'; return; } if (data.error) { document.getElementById('facModalContent').innerHTML = '<div style="text-align:center;padding:30px;color:#ef4444;">' + data.error + '</div>'; return; } renderFacultyReviews(data); })
...
window.loadFacultySummary = function(fid) {
    ...
    fetch(window.FACULTY_SUMMARY_URL + '?faculty_id=' + fid + '&action=summary', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.text()).then(text => { try { const d = JSON.parse(text); if (d.error === 'session_expired') { window.location.href = 'login.php?timeout=1'; return; } if (d.error) { el.innerHTML = '<div style="padding:20px;color:#ef4444;">Error: ' + d.error + '</div>'; return; } el.innerHTML = '<div style="padding:4px 0 16px;"><div style="background:var(--gray-100);border-radius:10px;padding:18px;font-size:13px;line-height:1.85;color:var(--gray-700);white-space:pre-wrap;border:1px solid var(--gray-200);">' + d.summary.replace(/</g,'&lt;') + '</div></div>'; } catch(e) { el.innerHTML = '<div style="padding:20px;color:#ef4444;">Server error.</div>'; } })
...
window.loadUserReviews = function(uid, page) {
    ...
    fetch(window.USER_REVIEWS_URL + '?user_id=' + uid + '&page=' + page, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.json()).then(data => {
            ...
            data.reviews.forEach(rev => {
                ...
                html += `<div style="padding:8px 0 10px;border-bottom:1px solid var(--gray-100);"><div style="display:flex;gap:8px;align-items:flex-start;"><div class="fac-rev-card" style="flex:1;" onclick='openRevDetail(${rd})'><div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;flex-wrap:wrap;"><span style="font-size:12px;font-weight:600;color:var(--gray-700);">${rev.faculty_name}</span><span class="badge ${sc}" style="font-size:10px;">${sl}</span><span style="font-size:10px;padding:1px 6px;border-radius:20px;font-weight:600;background:${stBg};color:${stC};">${rev.status.charAt(0).toUpperCase()+rev.status.slice(1)}</span><span style="font-size:10px;color:var(--gray-400);margin-left:auto;">${rev.created_at}</span></div><div style="font-size:12px;color:var(--gray-600);line-height:1.5;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${rev.review_text}</div></div><button type="button" onclick="deleteUserReview(${rev.id},${uid},${page})" style="flex-shrink:0;background:none;border:1px solid var(--gray-200);border-radius:6px;padding:5px 8px;cursor:pointer;color:var(--gray-400);font-size:11px;display:flex;align-items:center;gap:3px;" onmouseover="this.style.borderColor='#ef4444';this.style.color='#ef4444';" onmouseout="this.style.borderColor='var(--gray-200)';this.style.color='var(--gray-400)';"><svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button></div></div>`;
            });
            list.innerHTML = html;
...
window.deleteUserReview = function(rid, uid, pg) {
    ...
    fetch(window.FACULTY_REVIEWS_URL + '?faculty_id=0&action=delete_any&review_id=' + rid, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.json()).then(d => { if (d.success) loadUserReviews(uid, pg); else alert('Failed to delete.'); })
...
window.generateFacultySummary = function() {
    ...
    fetch('faculty_summary.php?month=' + month + '&year=' + year, { credentials: 'include', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.text()).then(text => {
            try { const d = JSON.parse(text); if (d.error === 'session_expired') { window.location.href = 'login.php?timeout=1'; return; } if (d.error) { el.innerHTML = '<span style="color:#ef4444;">Error: ' + d.error + '</span>'; } else { el.style.whiteSpace = 'pre-wrap'; el.textContent = d.summary || 'No summary available.'; } }
            catch(e) { el.innerHTML = '<span style="color:#ef4444;">Server error.</span>'; }
            btn.innerHTML = 'Regenerate'; btn.disabled = false;
        }).catch(() => { el.innerHTML = '<span style="color:#ef4444;">Network error.</span>'; btn.innerHTML = 'Generate'; btn.disabled = false; });
};
...
<script>
  window.SESSION_IDLE_TIMEOUT = <?= defined('SESSION_IDLE_TIMEOUT') ? SESSION_IDLE_TIMEOUT : 300 ?>;
