...
.sidebar {
    ...
    padding: 0 0 20px;
    box-shadow: 2px 0 16px rgba(139,0,0,0.22);
...
/* Logo area — center in collapsed width */
...
body.sidebar-collapsed .sidebar-logo-fallback {
    margin: 0 auto;
}

/* Avatar — shrink and center */
body.sidebar-collapsed .sidebar-user-wrap {
    padding: 10px 0 10px;
    display: flex;
    justify-content: center;
    align-items: center;
...
/* Nav links — center icon, same width as avatar/logo */
body.sidebar-collapsed .sidebar a {
    justify-content: center;
    padding: 10px 0;
    gap: 0;
}
body.sidebar-collapsed .sidebar a svg { margin: 0; }

/* Badge — hide inline badge; shown as overlay via admin.css for pending */
body.sidebar-collapsed .sidebar-nav-badge  { display: none; }

/* Footer logout link */
body.sidebar-collapsed .sidebar-footer { padding: 10px 0 0; }
body.sidebar-collapsed .sidebar-footer a { justify-content: center; padding: 10px 0; }

/* Toggle button */
.sidebar-toggle {
    position: fixed; top: 16px; left: calc(var(--sidebar-w) - 14px);
    z-index: 300; width: 28px; height: 28px;
    background: white; border: 1.5px solid var(--gray-200);
    border-radius: 50%; cursor: pointer;
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 2px 8px rgba(0,0,0,0.12);
    transition: left var(--transition), transform 0.2s;
    color: var(--maroon);
}
body.sidebar-collapsed .sidebar-toggle { left: calc(var(--sidebar-collapsed-w) - 14px); }
.sidebar-toggle:hover { background: var(--maroon-pale); transform: scale(1.1); }
...
.sidebar-brand-sub {
    ...
    font-weight: 400; opacity: 0.55; display: block; margin-top: 1px;
}
...
.sidebar-avatar {
    ...
    display: block; margin: 0 auto 8px; object-fit: cover;
    transition: width 0.28s, height 0.28s;
}
.sidebar-name { color: white; font-size: 14px; font-weight: 600; margin-bottom: 2px; white-space: nowrap; transition: opacity 0.2s; }
.sidebar-role { color: rgba(255,255,255,0.55); font-size: 11px; text-transform: uppercase; letter-spacing: 1px; white-space: nowrap; transition: opacity 0.2s; }

.sidebar nav { flex: 1; padding: 8px 10px; overflow-y: auto; overflow-x: hidden; }
.nav-label {
    font-size: 10px; text-transform: uppercase; letter-spacing: 1.2px;
    color: rgba(255,255,255,0.38); padding: 0 6px;
    margin: 12px 0 4px; white-space: nowrap;
...
.sidebar a svg { flex-shrink: 0; opacity: 0.85; min-width: 16px; }
.nav-link-text { transition: opacity 0.2s, width 0.28s; overflow: hidden; }

.sidebar-footer { border-top: 1px solid rgba(255,255,255,0.12); padding: 12px 10px 0; overflow: hidden; }
.sidebar-badge-wrap { margin-left: auto; }

/* ── Main content area ────────────────────────────────────── */
.main { margin-left: var(--sidebar-w); padding: 32px 32px 60px; min-height: 100vh; transition: margin-left var(--transition); }
...
.badge-toxic    { background: #fff7ed; color: #c2410c; }
...
/* ── Alerts ───────────────────────────────────────────────── */
.alert { padding: 13px 16px; border-radius: var(--radius-sm); font-size: 14px; margin-bottom: 20px; display: flex; align-items: center; gap: 8px; }
...
.modal-header h3, .modal-head h3 { font-size: 17px; font-weight: 600; color: var(--gray-800); }
.modal-close {
...
.form-group textarea {
    ...
    color: var(--gray-800); outline: none; transition: border-color 0.2s; background: white;
}
.form-group input:focus,
.form-group select:focus,
.form-group textarea:focus { border-color: var(--maroon); box-shadow: 0 0 0 3px rgba(139,0,0,0.07); }
...
.stat-card {
    ...
    border: 1px solid var(--gray-200); border-top: 3px solid transparent;
...
.notif-dropdown {
    ...
    width: 330px; box-shadow: var(--shadow-lg); z-index: 200; overflow: hidden;
}
...
.admin-table th { background: var(--gray-100); color: var(--gray-600); font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; padding: 11px 16px; text-align: left; border-bottom: 1px solid var(--gray-200); }
.admin-table td { padding: 13px 16px; border-bottom: 1px solid var(--gray-100); font-size: 14px; color: var(--gray-700); vertical-align: middle; }
.admin-table tr:last-child td { border-bottom: none; }
.admin-table tbody tr:hover td { background: #fafafa; }

/* ── Pagination ───────────────────────────────────────────── */
.pagination { display: flex; align-items: center; justify-content: center; gap: 6px; margin: 18px 0 28px; }
.page-btn {
...
.user-info-text strong { display: block; font-size: 14px; font-weight: 600; color: var(--gray-800); }
...
.stars label { font-size: 28px; cursor: pointer; color: #d1d5db; transition: color 0.15s; line-height: 1; }
.stars label:hover,
.stars label:hover ~ label,
.stars input:checked ~ label { color: #f59e0b; }
.rating-group { margin-bottom: 14px; }
.rating-group-label { font-size: 14px; font-weight: 500; color: var(--gray-700); margin-bottom: 6px; display: flex; justify-content: space-between; align-items: center; }
.rating-group-label span { font-size: 12px; color: var(--gray-400); font-weight: 400; }
.rating-section-title { font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: var(--gray-400); font-weight: 600; margin: 16px 0 10px; padding-bottom: 6px; border-bottom: 1px solid var(--gray-100); }
...
.search-box { display: flex; align-items: center; gap: 6px; background: var(--gray-100); border: 1px solid var(--gray-200); border-radius: 8px; padding: 7px 12px; min-width: 220px; max-width: 280px; }
.search-box input { border: none; background: transparent; outline: none; font-size: 14px; font-family: 'DM Sans', sans-serif; color: var(--gray-800); width: 100%; }
.search-box input::placeholder { color: var(--gray-400); }
.filter-sel { padding: 7px 10px; border: 1px solid var(--gray-200); border-radius: 8px; font-size: 13px; font-family: 'DM Sans', sans-serif; color: var(--gray-600); background: white; outline: none; cursor: pointer; }
.filter-sel:focus { border-color: var(--maroon); }
.no-results-row td, .empty-state-row td { text-align: center; padding: 28px; color: var(--gray-400); font-size: 14px; }
